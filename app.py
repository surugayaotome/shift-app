...
if st.session_state.user["is_admin"]:
    ...
    # --- タブ4: Excel出力（Image 2再現版） ---
    with tab4:
        st.write("### 📊 Excel出力")
        mode = st.radio("形式", ["日別", "週間タイル(Image 2)"])
        ex_date = st.date_input("出力開始日", value=datetime.date.today())
        ...
        def write_excel_day(ws, r_start, c_start, target_d):
            ...
            headers = ["ID", "氏名", "勤h", "休h", "希"] + time_slots + ["計"]
            ...
            for s in staff_list:
                ...
                else: vals = [s_id, s, 0, 0, ""] + [""] * len(time_slots) + [0]
                ...
                for i, v in enumerate(vals):
                    ...
                    if 5 <= i < 5 + len(time_slots) and v in ['1','2','同']: total_slots[i-5] += 1
                curr += 1
            ws.cell(curr, c_start+1, "合計").fill = PatternFill(start_color="FFFF00", fill_type="solid")
            for i, val in enumerate(total_slots): ws.cell(curr, c_start+5+i, val).fill = PatternFill(start_color="FFFF00", fill_type="solid")
            return curr + 2
        ...
        if st.button("Excel作成"):
            ...
            if mode == "日別": write_excel_day(ws, 2, 1, ex_date)
            else:
            ...
            wb.save(output)
            st.download_button("📥 ダウンロード", output.getvalue(), f"Shift_{ex_date.strftime('%m%d')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# 📱 従業員メニュー
    ...
    if ps and pe:
        ...
        for dstr in pd.date_range(ps, pe).strftime("%Y-%m-%d"):
            with st.expander(f"📅 {dstr}"):
                hope = st.text_input("希望 (OFF, 10-15など)", key=f"h_{dstr}")
                if st.button("保存", key=f"b_{dstr}"):
                    with engine.begin() as conn:
